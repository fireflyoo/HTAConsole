;(function(context) {
  ...
  var hyphen = function(input) {
    return input.replace(/([a-z][A-Z])/g, function (group) {
      return group.charAt(0) + '-' + group.charAt(1).toLowerCase()
    })
  }
  ...
  htaConsole.init = function() {
      ...
      document.getElementById('panel-dragbar').attachEvent('onmousedown', function(evt) {
        evt.preventDefault()
      ...
      // Focus
      document.getElementById('panel-console').attachEvent('onclick', function(evt) {
        document.getElementById('commandline').focus()
      })


      // keypress
      var self=document.getElementById('commandline')
      self.attachEvent('onkeypress', function(evt) {
        if (evt.keyCode === 13) {
          var sCmd = self.value
          if (sCmd === "clear") {
            htaConsole.clear()
            ...
            try {
	      jsSrc=sCmd.replace(/var[^\;]+;\n\n/, "");
              var evalCmd = eval(jsSrc)
      ...
      self.attachEvent('onkeydown', function(evt) {
        if (evt.keyCode === 38) {
          cmdHistoryPosition--
        ...
        if (evt.keyCode === 40) {
          ...
          if (cmdHistory[cmdHistoryPosition] !== undefined && cmdHistory[cmdHistoryPosition] !== '') {
            self.value = cmdHistory[cmdHistoryPosition]
            return false
          } else if (cmdHistoryPosition === cmdHistory.length) {
            self.value = ''
            return false
